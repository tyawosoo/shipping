...
# -------------------------
# 调试信息：展示读取到的表头与前几行，帮助定位列名差异
# -------------------------
st.markdown("---")
st.header("（调试）已读取 Excel 的表头与样本（请检查列名）")
st.write("车价表列名：", truck_df.columns.tolist())
st.write("车价表样本（前 5 行）：")
st.dataframe(truck_df.head(5))
st.write("箱子表列名：", box_df.columns.tolist())
st.write("箱子表样本（前 5 行）：")
st.dataframe(box_df.head(5))
